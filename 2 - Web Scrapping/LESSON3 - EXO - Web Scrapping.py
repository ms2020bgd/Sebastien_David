...
def get_prices_from_url(url):
    # Request
    requestPageObj = requests.get(URL_PAGE2)
    htmlPageObj = requestPageObj.text
    htmlParsedPageObj = BeautifulSoup(htmlPageObj, 'html.parser')
    
    # Prices
    tableOfPrices = htmlParsedPageObj.findAll("div", {"class": "pricing-table-header"})
    tableOfPrices = [price.text.split() for price in tableOfPrices]
    
    prices = []
    for priceTextList in tableOfPrices:
        for textEl in priceTextList:
            if '$' in textEl:
                prices.append(textEl)
    
    # Storage and databases
    tableOfContents = htmlParsedPageObj.findAll("ul", {"class": "pricing-table-list"})
    tableOfContents = [contents.text.split() for contents in tableOfContents]
    
    storageSize = []
    databaseNb = []
    for contentTextList in tableOfContents:
        for textEl in contentTextList:
            if ('GB' in textEl):
                storageSize.append(textEl)
            if ('GB' not in textEl) & (textEl.isnumeric()):
                databaseNb.append(textEl)
    
    # Offers
    offers = [offer.text for offer in htmlParsedPageObj.find_all('h2')]
    
    # Store results
    results = dict()        
    for offer, price, storage, database in zip(offers, prices, storageSize,
                                               databaseNb):
        
        tempDict = dict(price=price, storage=storage, databases=int(database))
        results[offer] = tempDict
    
    return results

results = get_prices_from_url(URL_PAGE2)
pprint(results)
